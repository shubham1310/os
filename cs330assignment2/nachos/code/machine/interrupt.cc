...
Interrupt::Halt()
{
    ...
    printf("CPU efficiency %lf\n",(1.0*totCPUBurst)/stats->totalTicks);
    printf("CPU burst length: Average= %lf max= %d  min=%d\n", sumBurst*1.0/coBurst,maxBurst, minBurst);
    printf("Number of non-zero CPU bursts observed(excluding main) = %d\n",coBurst);
     double avewait=(double)waitTime/(thread_index-1.0);
     //printf("TOT=%lld DIFF=%lld\n",totCPUBurst,DiffCPUBurst);
     printf("Average waiting time %lf \n",avewait);
     double error=(double)((1.0*DiffCPUBurst)/(1.0*totCPUBurst));
     if(SchedType==2)
     printf("Difference in S(n) and t(n) and efficiency= %lld %lf\n",DiffCPUBurst, error );
    //printf("Ave burst for testloop = %d == %d//%d \n", sumBurst/coBurst,sumBurst, coBurst);
     unsigned maX=CompletionTime[1],miN=CompletionTime[1];
     double AveCompTime=0.0;
     double SumCompTime=0.0;
     double AveCompTimeSquare=0.0;
     double SumCompTimeSquare=0.0;
     double VarCompTime=0.0;
     for(int i=1;i<thread_index;i++)
     {
        maX=max(maX,CompletionTime[i]);
        miN=min(miN,CompletionTime[i]);
        SumCompTime+=(1.0*CompletionTime[i]);
       // SumCompTimeSquare+=(1.0*CompletionTime[i]*CompletionTime[i]);
     }
     AveCompTime=(SumCompTime/(thread_index-1));
     for(int i=1;i<thread_index;i++)
     {
        SumCompTimeSquare+=((CompletionTime[i]-AveCompTime)*(CompletionTime[i]-AveCompTime));
     }
     //AveCompTimeSquare=(SumCompTimeSquare/(thread_index-1));
    // VarCompTime = AveCompTimeSquare-(AveCompTime*AveCompTime);
     VarCompTime=(SumCompTimeSquare/(thread_index-1));
    // if(SchedType>2)
    printf("Thread completion times:Average %lf Variance %lf max %u min %u\n\n\n",AveCompTime,VarCompTime,maX,miN);
