...
NachOSThread::NachOSThread(char* threadName)
{
    ...
    pid = thread_index;
    thread_index++;
    // Added--Initialise CPUBurst, waitInitial, S, initialTime and CPUsage for processes
    CPUBurst[pid]=0;
    CPUsage[pid]=0;
    initialTime=stats->totalTicks;
    waitInitial[pid]=0;
    S[pid]=0;
    // Initialise CPUBurst, waitInitial, S, initialTime and CPUsage for processes
...
void
NachOSThread::setStatus(ThreadStatus st)  //Now we will use only setStatus function to change status
{
    //We can easily check the change of status of threads.
    if(status!=RUNNING)
    {
        //If thread going to RUN update intialTime for thread and LastTime
        if(st==RUNNING)
        {
            LastTime=initialTime=stats->totalTicks;
        }
    }
    else if(status==RUNNING)
    {
        if(st!=RUNNING)
        {
            // If thread going from RUNNING to other state measure CPUBurst
            CPUBurst[pid]=stats->totalTicks-initialTime;
            //printf("PID = %d Cpu burst = %d\n",pid, CPUBurst[pid]);
            maxBurst=max(maxBurst,CPUBurst[pid]);
            minBurst=min(minBurst,CPUBurst[pid]);
            totCPUBurst+=CPUBurst[pid];
            if(pid>0)
            {
    ...
    {
        // Update time when thread entered in READY QUEUE
        waitInitial[pid]=stats->totalTicks;
    }
    else if(status == READY && st!=READY)
    {
        // increase waitTime if status changing from READY to ANY OTHER STATE
        if(pid>0) waitTime+=(stats->totalTicks - waitInitial[pid]);
    }
    // Change status 
    status = st;
}
...
NachOSThread::Exit (bool terminateSim, int exitcode)
{
    ...
    CompletionTime[pid]=stats->totalTicks;
    UpdateReq=true;
...
NachOSThread::PutThreadToSleep ()
{
   ...
   // status = BLOCKED;
    setStatus(BLOCKED);
    UpdateReq=true; // To stop multiple update of CPUsage on same context switch
    while ((nextThread = scheduler->FindNextToRun()) == NULL)
    {
      UpdateReq=false;
...
{
  // Change S to new value using previous CPUBurst
    if(CPUBurst[pid]==0) return ;
    //printf("PID:[%d] CPUBurst:[%d] S:[%d]\n",pid,CPUBurst[pid],S[pid]);
  DiffCPUBurst+=abs(CPUBurst[pid]-S[pid]);
